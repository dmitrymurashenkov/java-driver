...
public class ControlConnectionHandlerTest {
    ...
    public void getClusterInfo_should_return_cluster_info() throws ClusterNameMismatchException, InterruptedException {
        ...
        new RunClusterAssertion(scassandra) {
            @Override
            void performAssert(ScassandraCluster scassandra, ControlConnectionHandler connection) {
    ...
    private abstract static class RunClusterAssertion {
        ...
        private void run(ScassandraCluster scassandra, PeerRowValidator rowValidator) throws ClusterNameMismatchException, InterruptedException {
            ...
            ControlConnectionHandler connection = null;
            try {
                scassandra.init();
                cluster.init();
        ...
        ControlConnectionHandler openConnection(ScassandraCluster scassandra, Cluster cluster, PeerRowValidator rowValidator) throws ClusterNameMismatchException, InterruptedException {
            Connection connection = cluster.manager.connectionFactory.open(cluster.getMetadata().getHost(scassandra.address(1)));
            return new ControlConnectionHandler(connection, cluster.manager.protocolVersion(), new MetadataParser(cluster.getMetadata(), cluster.manager), rowValidator);
        }

        abstract void performAssert(ScassandraCluster scassandra, ControlConnectionHandler connection);
    }
}
